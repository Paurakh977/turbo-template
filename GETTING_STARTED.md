...
- Stack: Turborepo 2.x - pnpm 8 - Next.js 16 - NestJS 11 - Prisma 7 (driver adapters) - Better Auth 1.x
...
| pnpm | 8.15.5 (repo-pinned) | workspace package manager |
...
```bash
corepack enable            # ensures pnpm@8.15.5 (per the packageManager field)
pnpm install
```
...
4. **Strict validation — no silent fallbacks.** The apps refuse to start when a
   ...
   - **API**: `apps/api/src/env.ts` + the Joi schema in `apps/api/src/app.module.ts`
     (`HOST`, `PORT`, `DATABASE_URL`, `REDIS_URL`, `NEXT_PUBLIC_APP_URL`,
     `BETTER_AUTH_URL`, `BETTER_AUTH_SECRET`, `APP_NAME`)
   - **Auth**: `packages/auth/src/auth.ts` (`BETTER_AUTH_URL`, `BETTER_AUTH_SECRET`,
     `NEXT_PUBLIC_APP_URL`, `APP_NAME`, `EMAIL_FROM`). OAuth/email variables are
     optional — an empty value simply disables the feature.
   - **Web**: `apps/web/next.config.js` (`NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_APP_URL`,
     `BETTER_AUTH_URL`, `BETTER_AUTH_SECRET`; `NEXT_ALLOWED_DEV_ORIGINS` in dev;
     `INTERNAL_API_URL` in production)
   - **Seed**: `packages/database/src/env.ts` (`DATABASE_URL`, `SEED_ADMIN_*`,
     `BETTER_AUTH_URL`)
...
| `pnpm format` | Prettier on all `*.{ts,tsx,md}` |
...
- `globalPassThroughEnv: ["*"]` — all env vars are visible to tasks.
