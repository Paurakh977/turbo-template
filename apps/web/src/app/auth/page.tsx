...
export default function AuthPage() {
  ...
  const handleGoogleSignIn = async () => {
    await authClient.signIn.social({ provider: "google", callbackURL: `${APP_URL}/dashboard` });
  };

  const handleGithubSignIn = async () => {
    await authClient.signIn.social({ provider: "github", callbackURL: `${APP_URL}/dashboard` });
  };
        ...
        >
          <div className="text-center">
            ...
            <p className="text-muted-foreground text-sm mb-8 leading-relaxed">
              We sent a verification link to <strong className="text-foreground font-medium">{email}</strong>.
              Click the link to activate your account.
            </p>

            {resendSent ? (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-green-500 text-sm font-medium mb-4 bg-green-500/10 py-2.5 rounded-xl border border-green-500/20">
            ...
            <button
              onClick={() => { setMode("signin"); setError(""); setResendSent(false); }}
      ...
      >
        <div className="mb-8 text-center">
          ...
          <p className="text-[13px] text-muted-foreground mt-2">
            {mode === "signin" ? "Enter your details to sign in" : "Enter your details to get started"}
          </p>
        </div>

        <div className="space-y-3 mb-6">
          <button onClick={handleGoogleSignIn} className="w-full flex items-center justify-center gap-3 py-2.5 px-4 bg-background border border-border/60 rounded-xl text-[13px] font-medium hover:bg-muted/50 transition-colors shadow-sm">
            <GoogleIcon /> Continue with Google
          </button>
          <button onClick={handleGithubSignIn} className="w-full flex items-center justify-center gap-3 py-2.5 px-4 bg-foreground text-background rounded-xl text-[13px] font-medium hover:opacity-90 transition-opacity shadow-sm">
        ...
        <div className="relative flex items-center mb-6">
          ...
          <span className="flex-shrink-0 mx-4 text-[11px] text-muted-foreground uppercase tracking-wider font-medium">or</span>
          <div className="flex-grow border-t border-border/60"></div>
        </div>
        ...
        <form onSubmit={handleEmailAuth} className="space-y-3.5">
          <AnimatePresence initial={false}>
            {mode === "signup" && (
              <motion.div key="name" variants={formVariants} initial="hidden" animate="visible" exit="exit">
                <input
          ...
          <input
            className="w-full px-4 py-3 bg-background/50 border border-border/60 rounded-xl text-[14px] outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/20 transition-all placeholder:text-muted-foreground/70"
            type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} minLength={8} required
          />

          <AnimatePresence>
            {error && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                <p className="text-red-500 text-xs mt-1 bg-red-500/10 p-2 rounded-lg border border-red-500/20">{error}</p>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          >
            {loading ? <span className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin"></span> : (mode === "signin" ? "Sign In" : "Create Account")}
          </button>
        </form>
        ...
        <div className="mt-6 text-center space-y-2">
          <p className="text-[13px] text-muted-foreground">
            ...
            <button onClick={() => { setMode(mode === "signin" ? "signup" : "signin"); setError(""); }} className="text-foreground font-medium hover:underline">
              {mode === "signin" ? "Sign up" : "Sign in"}
            </button>
          </p>
          {mode === "signin" && (
            <p className="text-[13px]">
              <a href="/auth/forgot-password" className="text-muted-foreground hover:text-foreground transition-colors font-medium">
