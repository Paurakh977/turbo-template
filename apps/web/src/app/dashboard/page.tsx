        ...
        <div className="max-w-[760px] mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {isAdmin && (
              <Link
                href="/admin"
                className="text-sm px-4 py-2 bg-primary text-primary-foreground rounded-xl font-medium hover:bg-primary/90 transition-colors"
              >
                Admin Panel →
              </Link>
            )}
      ...
      <main className="flex-1 max-w-[760px] w-full mx-auto px-6 py-12">
        ...
        >
          ...
          <motion.div
            ...
            className="bg-card border border-border/50 rounded-[20px] p-6 sm:p-8 flex items-center gap-5 sm:gap-6 shadow-sm"
          >
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-xl sm:text-2xl font-semibold truncate mb-1">
                {session.user.name}
              </h2>
              <p className="text-muted-foreground text-[14px] truncate mb-2">
                {session.user.email}
              </p>
              <div className="flex items-center gap-2">
                <span
                  className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold tracking-wide uppercase ${session.user.emailVerified ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 'bg-yellow-500/10 text-yellow-600 border border-yellow-500/20'}`}
                >
                  {session.user.emailVerified ? 'Verified' : 'Unverified'}
                </span>
                <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold tracking-wide uppercase bg-blue-500/10 text-blue-600 border border-blue-500/20">
                  Role: {role}
                </span>
              </div>
            </div>
          ...
          <motion.div
            variants={itemVariants}
            className="bg-card border border-border/50 rounded-[20px] p-6 sm:p-8 shadow-sm"
          >
            ...
            {!hasPasswordAccount && hasFetchedAccounts ? (
              <div className="bg-secondary/50 border border-border/50 rounded-xl p-5 text-[13px] text-muted-foreground leading-relaxed">
                ...
                <strong className="text-foreground">
                  {userAccounts[0]?.providerId === 'google'
                    ? 'Google'
                    : 'GitHub'}
                </strong>
                ...
                <div>
                  ...
                  <p className="text-[13px] text-muted-foreground">
                    {(session.user as any).twoFactorEnabled
                      ...
                      : 'Not currently enabled on your account.'}
                  </p>
                </div>
                {(session.user as any).twoFactorEnabled ? (
                  <button
                    onClick={handleDisable2FA}
                    className="whitespace-nowrap px-4 py-2 border border-red-500/30 text-red-500 bg-red-500/5 hover:bg-red-500/10 rounded-lg text-[13px] font-medium transition-colors"
                  >
                    Disable 2FA
                  </button>
                ) : (
                  <button
                    ...
                    className="whitespace-nowrap px-4 py-2 bg-foreground text-background hover:bg-foreground/90 rounded-lg text-[13px] font-medium transition-colors shadow-sm"
                  >
                    Enable 2FA
                  </button>
                )}
          ...
          {isAdmin && (
            ...
            >
              ...
              <div className="flex gap-3 flex-wrap">
                <Link
                  href="/admin"
                  className="text-sm px-3 py-1.5 bg-primary/10 text-primary rounded-lg hover:bg-primary/20 transition-colors font-medium"
                >
                  User Management
                </Link>
                <Link
            ...
            >
              ...
              {setupStep === 'password' && (
                <div className="space-y-4">
                  <p className="text-[13px] text-muted-foreground">
                    Enter your password to verify it's you before proceeding.
                  </p>
                  <input
                    type="password"
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      ...
                      className="flex-1 py-2.5 bg-primary text-primary-foreground rounded-xl text-[13px] font-semibold hover:bg-primary/90 transition-all"
                    >
                      Continue
                    </button>
                    <button
                      onClick={() => setShow2FASetup(false)}
                      className="flex-1 py-2.5 bg-secondary text-secondary-foreground rounded-xl text-[13px] font-medium hover:bg-secondary/80 transition-all border border-border/40"
                    >
                      Cancel
                    </button>
                  </div>
              ...
              {setupStep === 'qr' && (
                <div className="space-y-5">
                    ...
                    <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 mt-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {backupCodes.map((code: any, i: any) => (
                          <code
                            key={i}
                            className="bg-background/80 border border-border/40 rounded py-1 px-2 text-[11px] font-mono text-center text-foreground/80"
                          >
                            {code}
                          </code>
                        ))}
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-primary text-primary-foreground rounded-xl text-[14px] font-semibold hover:bg-primary/90 transition-all mt-2"
                  >
                    Verify & Enable
                  </button>
                </div>
